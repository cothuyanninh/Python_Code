...
parser.add_argument( '--api-key', dest='apiKey', action='store', required=True, help='Google Data API key to use. You can get one here: https://console.developers.google.com')
parser.add_argument( '--channel', dest='channel', default = "UC9WayAVqWKIoyg1eN28n9Ug" ,action='store',help='Youtube channel to get videos from')
parser.add_argument( '--output-file-path', dest='outputFilePath', action='store', default='', help='File to write found video links to (content replaced each time). If this option is not specified, the links are sent to the standard output')
